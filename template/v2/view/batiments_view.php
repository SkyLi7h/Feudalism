...
<?php
	...
	foreach ($BATIMENTS as $batiment)
	{
		...
		else
		{
			...
			echo '<div class="layoutBatiment">';
				echo'<div id="nomBatiment">'.$batiment["nom"].'</div>';
				echo'<div id="niveauBatiment"><img width="10px" src="template/'. $TEMPLATE .'/images/up.png"> : '. $village->GetNiveauByName($batiment["nom"]) .' => <b><span style="color: green;">'. ($village->GetNiveauByName($batiment["nom"])+1) .'</span></b></div>';
				if($batiment["type"] == "Ressource")
				{
					echo'<div id="gainBatiment"><img width="20px" src="template/'. $TEMPLATE .'/images/'.$batiment["imgRes"].'"> : '. floor($gainSecondeBat[$batiment["nom"]]*3600) .' => <b><span style="color: green;"> '. floor($gainSecondeBatSuiv[$batiment["nom"]]*3600) .'</span></b></div>';
				}	
				echo'<div id="imgBatiment"><img src="template/'. $TEMPLATE .'/images/buildings/'.$batiment["img"].'"></div>';
				echo'<div id="descBatiment">'.$batiment["description"].'</div>';		
				echo $boutonConstruire;
